...
export function safeSheetNameForXLSX(name, fallback = "Sayfa") {
  const cleaned = (name ?? "").toString().replace(/[\\\/?*\[\]:]/g, " ").trim() || fallback;
...
export default function App() {
  ...
  const teachers = [
    { id: 1, name: "HAYATİ GÜLDAL" },
    { id: 2, name: "NECİP MURAT UYSAL" },
  ];
  const users = [
    ...teachers.map((t) => ({ name: t.name, role: "teacher" })),
    { name: "AYHAN KAPICI (Rehber Öğretmen)", role: "admin" },
  ];
  const credentials = {
    "HAYATİ GÜLDAL": "1234",
            ...
            <div className="flex items-center gap-2">
              {currentTeacher && (
                <div className="flex items-center gap-2 rounded-xl border border-gray-200 px-3 py-1 text-xs dark:border-gray-800">
                  <span>
                    Giriş: <span className="font-semibold">{currentTeacher}</span>
                  </span>
                  {currentRole === "admin" && (
                    <span className={`flex items-center gap-1 rounded-lg bg-blue-100 px-2 py-0.5 text-[10px] font-semibold text-blue-700 dark:bg-blue-900 dark:text-blue-200 ${adminGlow ? "shadow-md ring-2 ring-blue-400/50 dark:ring-blue-500/70" : ""}`}>
                      <ShipWheel size={12} className="text-blue-600 dark:text-blue-300" /> Yönetici Paneli
                    </span>
                  )}
                </div>
              )}
          ...
          <aside className="rounded-2xl border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-gray-900">
            ...
            <ul className="space-y-1 text-sm">
              {[...teachers.map(t => ({name: t.name, role: "teacher"})), {name: "AYHAN KAPICI (Rehber Öğretmen)", role: "admin"}].map((u, idx) => (
                <li key={idx} className={u.role === "admin" ? "flex items-center gap-1 font-semibold text-blue-600 dark:text-blue-400" : ""}>
                  {u.role === "admin" && <ShipWheel size={12} className={`inline-block text-blue-500 dark:text-blue-300 ${adminGlow ? "drop-shadow" : ""}`} />}
                  {" "}{u.name}
                </li>
              ))}
            </ul>
          ...
          <section className="space-y-4 rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
            {!currentTeacher ? (
              <LoginCard
                ...
                onSuccess={(name, role) => { setCurrentTeacher(name); setCurrentRole(role); }}
              />
            ) : (
              <>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                    <span>Rol: <strong>{currentRole || "teacher"}</strong></span>
...
function LoginCard({ users, credentials, onSuccess }) {
  ...
  const submit = (e) => {
    ...
    if (ok && pwd === ok) onSuccess(selected, users.find((u) => u.name === selected)?.role || "teacher");
      ...
      <form onSubmit={submit} className="space-y-2 text-sm">
        <div>
          ...
          <select value={selected} onChange={(e) => setSelected(e.target.value)}
            ...
            {users.map((u, idx) => (<option key={idx} value={u.name}>{u.name}</option>))}
          </select>
        </div>
        <div>
        ...
        {err && <div className="rounded-lg border border-rose-300 bg-rose-50 p-2 text-xs text-rose-700 dark:border-rose-900/50 dark:bg-rose-900/20 dark:text-rose-200">{err}</div>}
        <div className="pt-1">
          <button type="submit" className="w-full rounded-xl bg-gray-900 px-4 py-2 text-sm font-semibold text-white transition hover:opacity-90 dark:bg-white dark:text-gray-900">Giriş</button>
        </div>
      </form>
    </div>
  );
...
function EtutTable({ teachers, currentTeacher, currentRole, studentDb, onUploadExcel }) {
  ...
  const isCellAssignedToMe = (hour, col) => (dayCells[`${hour}-${col}-teacher`] || "") === currentTeacher;
  const cellVisible = (hour, col) => (currentRole === "admin" || !onlyMine) ? true : isCellAssignedToMe(hour, col);
  ...
  }, [selectedDate, rooms.length]); // eslint-disabled: exhaustive-deps
  ...
  const onStudentNameChange = (hour, col, part, name) => {
    ...
    if (q) {
      ...
      let match = entries.find(([_, v]) => (v.name || "").toLocaleUpperCase("tr-TR") === q);
      if (!match) {
        const starts = entries.filter(([_, v]) => (v.name || "").toLocaleUpperCase("tr-TR").startsWith(q));
  ...
  const filteredStudents = Object.entries(studentDb)
    ...
    .filter((s) => {
      ...
      if (!q) return true;
      return s.no.toLocaleLowerCase("tr-TR").includes(q) || s.name.toLocaleLowerCase("tr-TR").includes(q);
    })
  ...
  const exportCSV = () => {
    ...
    const header = ["Tarih", "Saat", "Salon", "Öğretmen", "Öğrenci No", "Öğrenci Adı", "Sınıf"];
    const lines = [header.join(",")].concat(
      rows.map((r) =>
        [r.date, r.hour, r.room, r.teacher, r.student_no, r.student_name, r.class]
  ...
  const exportXLSX = () => {
    ...
    const wsAll = XLSX.utils.json_to_sheet(rows, {
      header: ["date", "hour", "room", "teacher", "student_no", "student_name", "class"],
    });
    XLSX.utils.sheet_add_aoa(wsAll, [["Tarih", "Saat", "Salon", "Öğretmen", "Öğrenci No", "Öğrenci Adı", "Sınıf"]], { origin: "A1" });
    XLSX.utils.sheet_add_json(wsAll, rows, { origin: "A2", skipHeader: true });
    ...
    Object.entries(byTeacher).forEach(([teacher, list], idx) => {
      ...
      XLSX.utils.sheet_add_aoa(ws, [["Tarih", "Saat", "Salon", "Öğretmen", "Öğrenci No", "Öğrenci Adı", "Sınıf"]], { origin: "A1" });
  ...
  const exportPdf = () => {
      ...
      rooms.forEach((r, cIdx) => {
        ...
        if (teacher || s1 || s2) {
          entries.push({
            ...
            teacher,
            students: [
              s1 || s1no ? `${s1no ? s1no + " - " : ""}${s1}${s1c ? ` (${s1c})` : ""}` : null,
              s2 || s2no ? `${s2no ? s2no + " - " : ""}${s2}${s2c ? ` (${s2c})` : ""}` : null,
            ].filter(Boolean),
          });
        }
      });
    ...
    entries.forEach((e) => {
      if (e.teacher) teacherCount.set(e.teacher, (teacherCount.get(e.teacher) || 0) + 1);
      e.students.forEach((s) => studentCount.set(s, (studentCount.get(s) || 0) + 1));
    });
    ...
    const teacherRows = Array.from(teacherCount.entries())
      .sort((a, b) => b[1] - a[1])
      .map(([t, c]) => `<tr><td>${t}</td><td>${c}</td></tr>`)
      .join("") || '<tr><td colspan="2">Kayıt yok</td></tr>';
    const studentRows = Array.from(studentCount.entries())
      .sort((a, b) => b[1] - a[1])
    ...
    const detailRows = entries
      ...
      .map(
        (e) => `<tr><td>${e.hour}. Saat</td><td>${e.room}</td><td>${e.teacher || '-'}</td><td>${e.students.join(", ") || '-'}</td></tr>`
      )
    ...
    <div className="overflow-auto rounded-2xl border border-gray-200 dark:border-gray-800">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {currentRole !== "admin" && (
            <label className="ml-2 flex items-center gap-1 text-xs">
              <input type="checkbox" checked={onlyMine} onChange={(e) => setOnlyMine(e.target.checked)} /> Sadece benim atamalarım
            </label>
          )}
      ...
      {warnings.length > 0 && (
        <div className="mx-3 my-2 rounded-lg border border-amber-300 bg-amber-50 p-2 text-xs text-amber-800 dark:border-amber-900 dark:bg-amber-900/20 dark:text-amber-200">
          ...
          {warnings.length > 3 && <div>… {warnings.length - 3} benzer uyarı daha.</div>}
        </div>
      )}
      ...
      <div className="w-full overflow-x-auto">
        <table className="w-full min-w-[720px] table-fixed text-left text-sm">
              ...
              {rooms.map((room, idx) => (
                <th key={idx} className="px-3 py-3">
                  <div className="flex items-center gap-2">
                    <input
                      value={room}
                      onChange={(e) => setRooms((r) => r.map((x, i) => (i === idx ? e.target.value : x)))}
          ...
          <tbody>
            {hours.map((h) => (
              <tr key={h} className="border-b border-gray-100 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-900/60">
                <td className="px-3 py-2 text-xs text-gray-500">{h}. Saat</td>
                {rooms.map((_, c) => {
                  const filled = dayCells[`${h}-${c}-teacher`] || dayCells[`${h}-${c}-1-name`] || dayCells[`${h}-${c}-2-name`];
                  return (
                    <td key={`${h}-${c}`} className={`px-3 py-2 align-top ${filled ? "bg-green-50 dark:bg-green-900/10 rounded-lg" : ""}`}>
                      {!cellVisible(h, c) ? (
                          ...
                          {[1, 2].map((p) => (
                            <div key={p} className="flex items-center gap-1">
                              <input
                                ...
                                onChange={(e) => canEditThisCell() && onStudentNoChange(h, c, p, e.target.value)}
                                disabled={!canEditThisCell()}
                                className={`h-8 w-16 rounded-lg border px-2 text-xs outline-none transition focus:ring-2 dark:border-gray-700 dark:bg-gray-900 ${!canEditThisCell() ? "cursor-not-allowed bg-gray-50 text-gray-400" : "border-gray-300 focus:ring-gray-900/20"}`}
                              />
                              {(() => {
                                const no = dayCells[`${h}-${c}-${p}-no`];
                                const missing = no && !studentDb[no?.toString()?.trim?.()];
                                return missing ? (
                                  <button
                                    type="button"
                              ...
                              <input
                                ...
                                onChange={(e) => canEditThisCell() && onStudentNameChange(h, c, p, e.target.value)}
                                disabled={!canEditThisCell()}
                                className={`h-8 w-full rounded-lg border px-2 text-xs outline-none transition focus:ring-2 dark:border-gray-700 dark:bg-gray-900 ${!canEditThisCell() ? "cursor-not-allowed bg-gray-50 text-gray-400" : "border-gray-300 focus:ring-gray-900/20"}`}
                              />
                              <select
                                value={dayCells[`${h}-${c}-${p}-class`] || ""}
                                onChange={(e) => canEditThisCell() && setCell(`${h}-${c}-${p}-class`, e.target.value)}
                                disabled={!canEditThisCell() || !!dayCells[`${h}-${c}-${p}-class-auto`]}
                                title={dayCells[`${h}-${c}-${p}-class-auto`] ? "Sınıf otomatik dolduruldu (salt-okunur)" : ""}
                                className={`h-8 min-w-[76px] rounded-lg border px-2 text-xs outline-none transition focus:ring-2 dark:border-gray-700 dark:bg-gray-900 ${!canEditThisCell() || dayCells[`${h}-${c}-${p}-class-auto`] ? "cursor-not-allowed bg-gray-50 text-gray-400" : "border-gray-300 focus:ring-gray-900/20"}`}
                              >
                              ...
                              {currentRole === "admin" && dayCells[`${h}-${c}-${p}-class-auto`] && (
                                <button
                                  type="button"
                                  onClick={() => unlockClass(h, c, p)}
                                  className="h-8 px-2 shrink-0 rounded-lg border border-gray-300 text-xs transition hover:bg-gray-100 dark:border-gray-700 dark:hover:bg-gray-800"
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredStudents.map((s) => (
                    <tr key={s.no} className="border-t border-gray-100 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-900/60">
                      <td className="px-3 py-2">{s.no}</td>
                      <td className="px-3 py-2">{s.name}</td>
                      <td className="px-3 py-2">{s.class}</td>
                      <td className="px-3 py-2 text-right">
                        <button onClick={() => pickStudent(s)} className="rounded-lg border border-gray-300 px-2 py-1 text-xs hover:bg-gray-100 dark:border-gray-700 dark:hover:bg-gray-800">
                          Seç
                        </button>
                      </td>
                    </tr>
      ...
      <div className="border-t border-gray-100 p-3 text-xs text-gray-500 dark:border-gray-800">
        Not: Atamalar <strong>tarih bazlı</strong> tutulur. Excel (.xlsx) yüklediğinizde öğrencinin <strong>numarasını</strong> yazınca adı ve sınıfı otomatik dolar.
      </div>
    </div>
  );
}
...
function parseStudentExcel(file, setStudentDb) {
  ...
  reader.onload = (e) => {
    ...
    const rows = XLSX.utils.sheet_to_json(ws, { header: ["A", "B", "C"], defval: "" });
    const db = {};
...
function runSmokeTests() {
  try {
    ...
    expectTest("safeSheetName cleans illegal chars", !(/[\\\/?*\[\]:]/.test(safe)));
    expectTest("safeSheetName length <= 31", safe.length <= 31);
    expectTest("safeSheetName fallback when empty", safeSheetNameForXLSX("") === "Sayfa");
    expectTest("safeSheetName truncates long names", safeSheetNameForXLSX("X".repeat(60)).length === 31);
  } catch (e) {
    // eslint-disable-next-line no-console
